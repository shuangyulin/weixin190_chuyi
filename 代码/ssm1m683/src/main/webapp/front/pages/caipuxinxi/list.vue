<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.shiherenqun" type="text" placeholder="适合人群" ></input>
			</view>
        ...
        <view :style='categoryName===item.caipufenlei?sactiveItem:sitem' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.caipufenlei?'tabActive':''" @tap="categoryClick(item.caipufenlei)">{{item.caipufenlei}}</view>
      </scroll-view>
        ...
        <view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 1% 20rpx 2%","borderColor":"rgba(130, 163, 157, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","width":"47%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
                                <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.caipinmingcheng}}</view>
            ...
            <text v-if="userid && isAuth('caipuxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('caipuxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('caipuxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('caipuxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
      </view>
            <!-- 样式2 -->
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('caipufenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('caipufenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,caipufenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.caipufenlei = '%' + this.categoryName + '%'
				}
				...
				if(this.searchForm.shiherenqun){
					params['shiherenqun'] = '%' + this.searchForm.shiherenqun + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`caipuxinxi`, params);
                } else {
                    res = await this.$api.list(`caipuxinxi`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('caipuxinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.shiherenqun){
					searchForm['shiherenqun'] = '%' + this.searchForm.shiherenqun + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`caipuxinxi`, searchForm);
                } else {
                    res = await this.$api.list(`caipuxinxi`, searchForm);
                }
				// 如果是第一页数据置空
