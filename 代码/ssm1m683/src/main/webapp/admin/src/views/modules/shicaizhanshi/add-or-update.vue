<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="食材名称" prop="shicaimingcheng">
          <el-input v-model="ruleForm.shicaimingcheng" 
              placeholder="食材名称" clearable  :readonly="ro.shicaimingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="食材名称" prop="shicaimingcheng">
              <el-input v-model="ruleForm.shicaimingcheng" 
                placeholder="食材名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="食材分类" prop="shicaifenlei">
          <el-select :disabled="ro.shicaifenlei" v-model="ruleForm.shicaifenlei" placeholder="请选择食材分类">
            <el-option
                v-for="(item,index) in shicaifenleiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="食材分类" prop="shicaifenlei">
	      <el-input v-model="ruleForm.shicaifenlei"
                placeholder="食材分类" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.shicaitupian" label="食材图片" prop="shicaitupian">
          <file-upload
          tip="点击上传食材图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.shicaitupian?ruleForm.shicaitupian:''"
          @change="shicaitupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.shicaitupian" label="食材图片" prop="shicaitupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shicaitupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="食材功效" prop="shicaigongxiao">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="食材功效"
                  v-model="ruleForm.shicaigongxiao" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shicaigongxiao" label="食材功效" prop="shicaigongxiao">
                    <span>{{ruleForm.shicaigongxiao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="食材介绍" prop="shicaijieshao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.shicaijieshao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shicaijieshao" label="食材介绍" prop="shicaijieshao">
                    <span v-html="ruleForm.shicaijieshao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	shicaijieshao : false,
      ...
      rules: {
          ...
          shicaifenlei: [
                { required: true, message: '食材分类不能为空', trigger: 'blur' },
          ],
          shicaitupian: [
          ],
          shicaigongxiao: [
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shicaifenlei/shicaifenlei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shicaifenleiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.shicaijieshao = this.ruleForm.shicaijieshao.replace(reg,'../../../ssm1m683/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shicaitupian!=null) {
		this.ruleForm.shicaitupian = this.ruleForm.shicaitupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shicaizhanshi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shicaizhanshiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shicaitupianUploadChange(fileUrls) {
	this.ruleForm.shicaitupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
